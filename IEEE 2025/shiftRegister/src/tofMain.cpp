...
#define SEC_DELAY 1000   // Delay (in ms) between initializing each sensor
...
// initialize all the pins connected to the shift register as outputs
  ...
  Wire.begin();
    ...
    for (int i = 0; i < NUM_SENSORS; i++) {
      ...
      delay(500);  // Small delay to stabilize each sensor
  }
}
...
void loop() {
  ...
  for (int i = 0; i < NUM_SENSORS; i++) {
    // Simulate turning on the corresponding sensor
    storageByte |= newSensor;
  ...
  // Keep all sensors on in the final state
  updateShiftRegister(storageByte);
}
